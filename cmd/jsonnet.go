...
func JsonnetVM(cmd *cobra.Command) (*jsonnet.VM, error) {
	...
	for _, extvar := range extvarfiles {
		...
		if len(kv) != 2 {
			fatalog(err).Str("ext-str-file", extvar).Msg("Failed to parse. Missing '=' in parameter`")
		}
...
func renderJsonnet(cmd *cobra.Command, files []string, param string, prune bool, prepend string, source string) string {
	...
	if err != nil {
		fatalog(err).Msg("Error creating jsonnet VM")
	}
	...
	if err != nil {
		fatalog(err).Str("source:", "renderJsonnet>EvaluateAnonymousSnippet").Msg("Error evaluating jsonnet snippet")
...
var jsonnetrenderCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		case "yaml":
			...
			if err != nil {
				fatalog(err).Msg("Error converting JSON to YAML")
			}
		...
		default:
			fatalog(err).Msg("Output format must be json, yaml or stream")
		}
	},
}
