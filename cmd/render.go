...
func init() {
	...
	renderjsonnetCmd.PersistentFlags().Bool("prune", true, "Prune null and empty objects from rendered json")
	renderjsonnetCmd.PersistentFlags().String("clusterparams", "", "provide cluster params as single file - can be combined with --cluster to override cluster")
	renderjsonnetCmd.PersistentFlags().StringP("component", "C", "", "component to render params for")
	renderjsonnetCmd.PersistentFlags().StringP("format", "F", "json", "Output forma: json, yaml, stream")
	renderjsonnetCmd.PersistentFlags().StringP("cluster", "c", "", "cluster to render params for")
